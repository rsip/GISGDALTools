...
public class Gdalinfo {
	...
	public String usage(String params) {
	...
	public static void gdalinfoGeneral(String[] args) {
		{
			Dataset hDataset;
			Band hBand;
			int i, iBand;
			...
			GDALInfoReportCorner(hDataset, "Lower Left ", 0.0,
					hDataset.getRasterYSize());
			GDALInfoReportCorner(hDataset, "Upper Right",
					hDataset.getRasterXSize(), 0.0);
			GDALInfoReportCorner(hDataset, "Lower Right",
					hDataset.getRasterXSize(), hDataset.getRasterYSize());
			...
			for (iBand = 0; iBand < hDataset.getRasterCount(); iBand++) {
				...
				/*
				 * if( bSample ) { float[] afSample = new float[10000]; int 
				 *  nCount; nCount = hBand.GetRandomRasterSample( 10000,
				 * afSample ); System.out.println( "Got " + nCount + " samples."
				 * ); }
				 */
				...
				if (hBand.GetStatistics(bApproxStats, bStats, dfMin, dfMax,
						dfMean, dfStdDev) == gdalconstConstants.CE_None) {
					System.out.println("Minimum=" + dfMin[0] + ",Maximum="
							+ dfMax[0] + ",Mean=" + dfMean[0] + ",StdDev="
							+ dfStdDev[0]);
				}
				if (bReportHistograms) {
					int[][] panHistogram = new int[1][];
					int eErr = hBand.GetDefaultHistogram(dfMin, dfMax,
							panHistogram, true, new TermProgressCallback());
				...
				if (hBand.GetOverviewCount() > 0) {
					...
					if (bComputeChecksum) {
						...
						for (iOverview = 0; iOverview < hBand
				...
				if ((nMaskFlags & (gdalconstConstants.GMF_NODATA | gdalconstConstants.GMF_ALL_VALID)) == 0) {
					...
					if (hMaskBand != null && hMaskBand.GetOverviewCount() > 0) {
						...
						for (iOverview = 0; iOverview < hMaskBand
								.GetOverviewCount(); iOverview++) {
							Band hOverview;
							if (iOverview != 0)
								System.out.print(",");
				...
				if (hBand.GetUnitType() != null
						&& hBand.GetUnitType().length() > 0) {
				...
				if (papszCategories.size() > 0) {
					...
					while (eCategories.hasMoreElements()) {
						System.out.println("" + i + ":"
								+ (String) eCategories.nextElement());
						i++;
					}
				}
				...
				if (bShowRAT && rat != null) {
					...
					if (rat.GetLinearBinning(pdfRow0Min, pdfBinSize)) {
						System.out.print("Row0Min=\"" + pdfRow0Min[0]
								+ "\" BinSize=\"" + pdfBinSize[0] + "\">");
					}
					System.out.print("\n");
					int colCount = rat.GetColumnCount();
					for (int col = 0; col < colCount; col++) {
						System.out.println("<FieldDefn index=\"" + col + "\">");
						System.out.println("<Name>" + rat.GetNameOfCol(col)
								+ "</Name>");
						System.out.println("<Type>" + rat.GetTypeOfCol(col)
								+ "</Type>");
						System.out.println("<Usage>" + rat.GetUsageOfCol(col)
								+ "</Usage>");
					...
					for (int row = 0; row < rowCount; row++) {
						...
						for (int col = 0; col < colCount; col++) {
							System.out.println("<F>"
									+ rat.GetValueAsString(row, col) + "</F>");
						}
	...
	static boolean GDALInfoReportCorner(Dataset hDataset, String corner_name,double x, double y) {
		...
		if (pszProjection != null && pszProjection.length() > 0) {
			...
			if (hLatLong != null) {
				/*
				 * New in GDAL 1.10. Before was
				 * "new CoordinateTransformation(srs,dst)".
				 */
				hTransform = CoordinateTransformation
						.CreateCoordinateTransformation(hProj, hLatLong);
			}
			if (hProj != null)
				hProj.delete();
		}
