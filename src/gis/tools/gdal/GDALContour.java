...
public static void main(String[] args) {
		 ...
		 */
		Dataset dataset = gdal.Open(sourceFilename, gdalconstConstants.GA_ReadOnly);
		if (dataset == null) {
		...
		Layer layer = null;
		if (driver == null) {
			System.err.println("Unable to find format driver named "+outputFormat);
			System.exit(10);
		}
		...
		if (threeDimension) {
			layer = dataSource.CreateLayer(newLayerName,srs,ogr.wkbLineString25D);
		} else {
			layer = dataSource.CreateLayer(newLayerName,srs,ogr.wkbLineString);
		}
		if (layer == null) {
			System.exit(1);
		}
		...
		gdal.ContourGenerate(band,contourInterval,offset,fixedLevelsDouble,
				...
				(attributName != null ? feature.GetFieldIndex(attributName):-1),progressCallback);
		dataSource.delete();
		dataset.delete();
	}
