...
public static void mainOgrInfo(String[] args){
		...
		for (int i = 0; i < args.length; i++) {
			 ...
			 * sSystem.out.print(pszTemp, "DISPLAY_FIELDS=%s", papszArgv[iArg] +
			 * strlen("-fields=")); papszOptions = CSLAddString(papszOptions,
			 * pszTemp); CPLFree(pszTemp); } else if(
			 * EQUALN(papszArgv[iArg],"-geom=", strlen("-geom=")) ) { char*
			 * pszTemp = (char*)CPLMalloc(32 + strlen(papszArgv[iArg]));
			 * sSystem.out.print(pszTemp, "DISPLAY_GEOMETRY=%s", papszArgv[iArg]
			 * + strlen("-geom=")); papszOptions = CSLAddString(papszOptions,
			 * pszTemp); CPLFree(pszTemp); }
			 */
		...
		if (poDS == null) {
			System.out.print("FAILURE:\n" + "Unable to open datasource"
					+ pszDataSource + "' with the following drivers.\n");
		...
		if (pszSQLStatement != null) {
			...
			if (papszLayers.size() > 0)
				System.out
						.println("layer names ignored in combination with -sql.");
			Layer poResultSet = poDS.ExecuteSQL(pszSQLStatement,
					poSpatialFilter, pszDialect);
			if (poResultSet != null) {
				if (pszWHERE != null) {
					if (poResultSet.SetAttributeFilter(pszWHERE) != ogr.OGRERR_NONE) {
						System.err.println("FAILURE: SetAttributeFilter("
								+ pszWHERE + ")failed.");
						return;
					}
				}
		...
		for (int iRepeat = 0; iRepeat < nRepeatCount; iRepeat++) {
			if (papszLayers.size() == 0) {
				...
				for (int i = 0; i < papszLayers.size(); i++) {
					Layer poLayer = poDS.GetLayerByName((String) papszLayers
							.get(i));
					if (poLayer == null) {
						System.out
								.println("FAILURE: Couldn't fetch requested layer "
...
//						+ "datasource_name [layer [layer ...]]\n");
		...
		System.out.print("ogrinfo --help-general");
}
...
static void reportOnLayer(Layer poLayer,String pszWHERE,Geometry poSpatialFilter){
		...
		if (pszWHERE != null) {
			if (poLayer.SetAttributeFilter(pszWHERE) != ogr.OGRERR_NONE) {
				System.err.println("FAILURE: SetAttributeFilter(" + pszWHERE
						+ ") failed.");
				return;
			}
		}
		...
		if (bVerbose) {
			...
			if (oExt != null)
				System.out.println("Extent:(" + oExt[0] + "," + oExt[2] + ")-("
						+ oExt[1] + "," + oExt[3] + ")");
			String pszWKT;
			...
			if (poLayer.GetGeometryColumn().length() > 0)
				System.out.println("Geometry Column ="
		...
		if (nFetchFID == OGRNullFID && !bSummaryOnly) {
			...
			if (poFeature == null) {
				System.out.println("Unable to locate feature id " + nFetchFID
						+ " on this layer.");
			} else {
				poFeature.DumpReadable();
			}
